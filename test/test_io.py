...
def test_reduceColours_optimised():
	...
	"""
	assert(not isinstance(io.reduceColours(io.openImage(INPUT + "/test_field.png"), "optimised").getcolors(maxcolors=256), type(None)))

def test_reduceColours_logo():
	"""
	.getcolors with maxcolors 16
	"""
	assert(not isinstance(io.reduceColours(io.openImage(INPUT + "/test_field.png"), "logo").getcolors(maxcolors=16), type(None)))
